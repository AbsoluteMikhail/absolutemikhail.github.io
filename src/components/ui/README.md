...
Здесь собраны повторяющиеся элементы сайта: красно-оранжевые акценты, открытые текстовые блоки и крупные изображения проектов. Светлая и тёмная темы сохраняют одну компоновку; светлая палитра задана семантическими токенами на `html.light` в `src/index.css`.

Переключатель — `src/components/ThemeToggle.tsx` в шапках, состояние — `src/lib/theme.ts`. Выбор хранится под ключом `gamepunk-theme-v1` 180 дней; без сохранённого выбора используется системная тема. При недоступном системном предпочтении и при пререндеринге используется тёмная тема. `/snippet` всегда тёмный.
...
| Переключение светлой/тёмной темы | `src/components/ThemeToggle.tsx`; токены `html.light` в `src/index.css` |
