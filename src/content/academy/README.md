...
Renderer принимает только HTTPS-ссылки с `blueprintue.com`, преобразует их в интерактивный `/render/` viewer и показывает резервную ссылку для открытия в отдельной вкладке. Для долговечных материалов выбирай публикацию без срока удаления и сохраняй PNG-скриншот как резерв внешнего сервиса.
