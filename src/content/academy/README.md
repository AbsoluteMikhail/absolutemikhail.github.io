...
Первая строка блока — постоянная HTTPS-ссылка с `blueprintue.com`, вторая — необязательный локальный скриншот, третья — его alt-текст. Renderer преобразует ссылку в интерактивный `/render/` viewer. Если добавлен скриншот, читатель может явно переключаться между вкладками «Граф» и «Скриншот»; при ошибке загрузки iframe renderer также попробует показать локальную копию. Для долговечных материалов выбирай публикацию без срока удаления.
